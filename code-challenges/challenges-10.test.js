...
const getNames = (arr) => {
  // Solution code here...
  return arr.map(result => result.name.split('').reverse().join(''));
...
const count = (target, input) => {
  ...
  let array = input.map(element => {
    let arrays = element.filter(a => a == target)
    i = i + arrays.length;
  })
  return i;
...
const totalSum = (input) => {
  // Solution code here...
  let i = input.reduce((accumalator, arr) => {
    let array = arr.reduce((a, val) => a += val, 0);
    return accumalator += array;
  }, 0);
  return i;
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let array = input.map(a => a.filter(b =>typeof (b) === 'number' && !(b % 5)));
  let res = array.map(c => c.map(d => Math.pow(2, d)));
  return res;
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let array = data.filter(a => {
    if (a.gender === 'female' || a.gender === 'male') {
      return a.name;

    }
  }).map(b => b.name).join(' and ');
  return array;
...
let findShortest = (data) => {
  // Solution code here...
  let char = data.sort((a, b) => a.height - b.height);
  return char[0].name;
};
