...
const listFoods = (recipe) => {
  let result = [];
  let unitArr = ['medium-sized', 'pounds', 'gallons', 'cups', 'pound'];

  for (let i = 0; i < gruffaloCrumble.ingredients.length; i++) {
    unitArr.forEach(unit => {
      let indexOfUnit = recipe.ingredients[i].indexOf(unit);
      if (indexOfUnit != -1) {
        let slicedString = recipe.ingredients[i].slice(indexOfUnit);
        let ingred = slicedString.slice(slicedString.indexOf(' ') + 1);
